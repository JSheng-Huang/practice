...
class Solution:
    def findMedianSortedArrays(self, nums1, nums2):
        ...
        while l < r:
            ...
            """
            If `nums1[m1] < nums[m2 -1]`, it means the k-th smallest number in 
            two arrays may be in the range `nums1[:m1 + 1]`, else it may be in 
            the range `nums1[m1 + 1:]`, If `l` reaches `nums1` length, it means 
            the k-th smallest number is either the first/last number in `nums1` 
            or in `nums2`.
            """
        ...
        smallest number is not in `nums1` or `nums2`. If both `m1` and `m2` are 
        not equal to 0, choose the larger one as the answer.
        """
        ...
        if (nums1_len + nums2_len) % 2 == 1:
            return ans1
        """
        If the total length of two arrays are even, it needs to take another 
        one to average. If `m1` is equal to `nums1` length or `m2` is equal to 
        `nums2` length, it means that the k-th smallest number is not in 
        `nums1` or `nums2`. If the length of `nums1` and `nums2` is not equal 
        to 0, choose the smaller one to average.
        """
...
class Solution:
    ...
    def findKthSmallest(self, nums1, nums1_bgn, nums1_len, nums2, nums2_bgn, nums2_len, k):
        ...
        """Because the answer is not in `nums1`, just find it in `nums2`."""
        if nums1_len == 0:
        ...
        Compare the (k/2)-th smallest number between `nums1` and `nums2`, if 
        the (k/2)-th smallest number in `nums1` is smaller than the one in 
        `nums2`, then the k-th smallest number would not be in the top k/2 
        numbers in `nums1`, because the k-th smallest number is larger than the 
        (k/2)-th smallest number, and vice versa. The maximum of `k_1` is k/2, 
        because we always choose the smaller list as `nums1`.
        """
